...
var (
	...
	blobs struct {
		data.Table
		pos data.RowT[geom.Vec2[float32]]
...
func blobsCollide(a, b geom.Vec2[float32]) bool {
	sq := blobsCollideDist * blobsCollideDist
	return a.Minus(b).Len2() < float32(sq)
}

func blobCollidesWithAny(pos geom.Vec2[float32]) bool {
...
func blobsInRange(pos geom.Vec2[float32], radius float32) (s []geom.Vec2[float32]) {
	for _, p := range blobs.pos {
...
func update() {
	...
	for i, pos := range blobs.pos {
		if rand.Intn(blobsBreedOdds) == 0 {
			...
			childPos := pos.Plus(geom.Vec2RandNormal[float32]().ScaledBy(dist))
	...
	for i := range preds.ori {
		if rand.Intn(predsBreedOdds) == 0 {
			preds.Append(
				preds.ori[i].Vec2().Plus(geom.Vec2RandNormal[float32]().ScaledBy(10)).Ori2(),
				geom.Vec2RandNormal[float32](),
